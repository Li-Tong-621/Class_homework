...
def main():
    ...
    while True:
        ...
        myeye.con()
        if winner is None:
            click_point = MY._get_clickpoint()
            #print(1)
            # 点击区域有效
            if click_point is not None:
                ...
                if checkerboard.can_drop(click_point):
                    ...
                    if winner is None:
                        ...
                        newboard = np.loadtxt('newboard.txt', dtype=np.int, delimiter=',', unpack=False)
                        i, j = view.KK(newboard)
                        newboard[i][j] = 2
                        file1 = 'oldboard.txt'
                        file2 = 'newboard.txt'
                        np.savetxt(file1, newboard, fmt='%d', delimiter=',')
                        np.savetxt(file2, newboard, fmt='%d', delimiter=',')
                        AI_point = Point(i, j) #
