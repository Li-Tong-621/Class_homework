...
class DeepQNetwork:
    ...
    def choose_action(self,qipan, observation):  #根据当前的状态的到动作
        if np.random.uniform()<=self.epsilon:  #在该可能性下选择最优
            ...
            ckpt = tf.train.get_checkpoint_state('E:\\python_code\\LITONG_WUZIQITRY\\reinforcementlearning\\meta\\')
            ...
            with tf.Session() as sess:
                ...
                actions_value = sess.run(tf.convert_to_tensor(sess.run(tf.get_collection('eval_net_params')[0])),feed_dict={self.s:ob})[0]
                #actions_value = sess.run(self.q_eval, feed_dict={self.s:ob})[0]
