...
from Black_AI_alpha_beta  import *
...
def main():
    ...
    while True :
        ...
        b1 = 'E:/python_code/LITONG_WUZIQITRY/ES/b1.txt'
        b2 = 'E:/python_code/LITONG_WUZIQITRY/ES/b2.txt'
        b3 = 'E:/python_code/LITONG_WUZIQITRY/ES/b3.txt'
        b4 = 'E:/python_code/LITONG_WUZIQITRY/ES/b4.txt'
        b5 = 'E:/python_code/LITONG_WUZIQITRY/ES/b5.txt'
        ...
        if winner is not None:
            ...
            if count>24:
                ...
                bwin = sorted(bwin.items(), key=lambda x: x[1], reverse=True)  # 按字典集合中，每一个元组的第二个元素排列。
                ...
                np.savetxt('w3.txt', temp, fmt='%f', delimiter=',')
                    ##黑色调整
                temp = np.loadtxt(B[bwin[0][0]], dtype=np.float, delimiter=',', unpack=False)
                temp = temp.reshape((14, 1))
                np.savetxt('b1.txt', temp, fmt='%f', delimiter=',')
                # 最好的突变
                for i in range(14):
                    temp2 = random.uniform(-0.5 * temp[i], 0.5 * temp[i])
                    if temp2 < 1:
                        temp2 = 1
                    temp[i] = temp[i] + temp2
                np.savetxt('b4.txt', temp, fmt='%f', delimiter=',')

                temp = np.loadtxt(B[bwin[1][0]], dtype=np.float, delimiter=',', unpack=False)
                temp = temp.reshape((14, 1))
                np.savetxt('b2.txt', temp, fmt='%f', delimiter=',')
                # 最好的突变
                for i in range(14):
                    temp2 = random.uniform(-0.5 * temp[i], 0.5 * temp[i])
                    if temp2 < 1:
                        temp2 = 1
                    temp[i] = temp[i] + temp2
                np.savetxt('b5.txt', temp, fmt='%f', delimiter=',')

                temp = np.loadtxt(B[bwin[2][0]], dtype=np.float, delimiter=',', unpack=False)
                temp = temp.reshape((14, 1))
                np.savetxt('b3.txt', temp, fmt='%f', delimiter=',')
