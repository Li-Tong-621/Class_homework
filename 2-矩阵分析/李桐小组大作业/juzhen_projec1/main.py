		...
		if moving >= 0:
			if verticesMove[moving] == 1:
				...
				if a != x & b != y:
					...
					drawMesh_1.draw_mesh(new_vertices,edges,img)
...
if __name__ == "__main__":
	...
	f.close()
	filename=L
	yofile_object  = open(os.path.join(__location__,filename), 'r')
	no_vertices, no_faces, vertices, faces = drawMesh_1.read_file(yofile_object)
	verticesMove = np.zeros(no_vertices+1)
	verticesSelect = np.zeros(no_vertices+1)

	img = np.zeros((800, 1280, 3), np.uint8)
	img2 = img.copy()
	edges = drawMesh_1.get_edges(no_faces,faces)
	...
	count = 0
	drawMesh_1.draw_mesh(vertices,edges,img)
	cv2.namedWindow(windowName)
	while (True):
		...
		if cv2.waitKey(1) == 32:
			drawMesh_1.save_mesh(no_vertices, no_faces, new_vertices, faces, count)
			count +=1
