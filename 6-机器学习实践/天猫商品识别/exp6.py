...
class MYDataset(Dataset):
    def __init__(self,kind):
        ...
        if kind=='trian':
            ...
            #self.rpath='./data3/I/'
            self.path=ann_file_train
            self.rpath=img_spath
        else:
            #self.path='./data4/i_train.json'
            self.path=ann_file_test
            #self.rpath='./data4/I/'
            self.rpath=video_img_spath
...
model=model.to(device)
train(path=outpath,model=model,optimizerkind='Ranger',criterionkind='cross_entropy',warmkind='multistp')
